<template>
  <div class="about">
    ...
    <div>
      ...
      <form>
        <label for="request">Enter Your comment:</label>
        <input v-model="request" type="text" id="request" placeholder="comment"/>
        <button @click="toggleDone" type="button">Submit</button>
        <p v-if="done">THANK YOU FOR YOUR RESPONSE</p>
      </form>
    </div>
  </div>
</template>
